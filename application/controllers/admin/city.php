...
class City extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->page_dir = 'admin/city/';
        $this->load->model('gallery_model', 'gallery');
        $this->data['active_menu'] = 'City';
        $this->data['controler'] = 'city';
        $this->load->model('city_model', $this->data['controler']);
    ...
    function index() {
        ...
        $where = 'status = 1';
        $this->data ['list_data'] = $this->city->get_data(null, $where);
    ...
    function add() {
        ...
        if ($this->input->post('submit')) {
            $this->form_validation->set_rules('txt_name', 'City name', 'trim|xss_clean|required');
            if ($this->form_validation->run()) {
                $data = array(
                    'name' => $this->input->post('txt_name'),
                    'status' => 1
                );
                if ($this->city->add_data($data)) {
    ...
    function update() {
        $this->data['cat_id'] = $this->uri->segment(4);
        $this->data['rec_data'] = $this->city->get_data(null, array('id' => $this->data['cat_id']), null, null, null, null, 'row');
        $this->data ['navigation']['admin/' . $this->data['controler']] = $this->data['active_menu'];
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run()) {
                ...
                if ($this->city->edit_data($data, array('id' => $this->data['cat_id']))) {
    ...
    function delete() {
        ...
        $ubah = $this->city->edit_data(array('status' => 0), array('id' => $id));
        if ($ubah == true) {
