 ...
 */
class Event_comment extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->page_dir = 'admin/event_comment/';
        $this->data['active_menu'] = 'Event Comment';
        $this->data['controler'] = 'event_comment';
        $this->load->model('event_comment_model', 'comment');
    ...
    function index() {
        ...
        $this->data ['list_data'] = $this->comment->get_data(null, array('event_id' => $id, 'status' => 1));
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run()) {
                ...
                if ($this->comment->edit_data($data, array('id' => $id))) {
                    ...
                    redirect(site_url('admin/' . $this->data['controler'] . '/index/' . $this->data['rec_data']->event_id));
                } else {
                    $this->data ['form_validation_errors'] = get_messages('Failed to save Data', 'alert-success', 'text-muted');
                }
            } else {
    ...
    function delete() {
        ...
        redirect(site_url('admin/event_comment/index/' . $page));
    }
