...
class Story extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->page_dir = 'admin/story/';
        $this->data['active_menu'] = 'Story';
        $this->data['controler'] = 'story';
    ...
    function index() {
        ...
        $this->data ['list_data'] = $this->story->get_data(null, 'status !=0', null, null, null, 'id desc');
    ...
    function index_category() {
        ...
        $this->data ['page_title'] = 'List ' . $this->data['active_menu'];
        $this->data['info_messages'] = $this->session->flashdata('info_messages') ? get_messages(wrap_text($this->session->flashdata('info_messages')), 'alert-success') : null;
        $this->data ['list_data'] = $this->story->get_data(null, array('status' => 1, 'category_id' => $this->data['cat_id']));
        $this->data ['page_icon'] = 'icomoon-icon-list';
        $this->data ['page'] = $this->load->view($this->get_page(), $this->data, true);
        $this->render();
    }
    ...
    function add() {
        ...
        $this->data ['navigation']['admin/story/index/' . $this->data['cat_id']] = $this->data['active_menu'];
    ...
    function update() {
        ...
        $this->data ['list_image'] = $this->gallery->get_data(null, null, null, null, 'id DESC');
        $this->data['info_messages'] = $this->session->flashdata('info_messages') ? get_messages(wrap_text($this->session->flashdata('info_messages')), 'alert-success') : null;
        $this->data['rec_data'] = $this->story->get_data(null, array('id' => $id), null, null, null, null, 'row');
            ...
            if ($this->form_validation->run()) {
                if (!empty($_FILES['uploadimage']['name'])) {
                    ...
                    if ($this->upload->do_upload('uploadimage')) {
                        ...
                        if ($this->story->edit_data($data, array('id' => $id))) {
    ...
    function delete() {
        ...
        $ubah = $this->story->edit_data(array('status' => 0), array('id' => $id));
        if ($ubah == true) {
            $this->session->set_flashdata('info_messages', 'Successfully Deleted');
        } else {
            $this->session->set_flashdata('info_messages', 'Fail Deleted');
        }
        redirect(site_url('admin/story/index/' . $page));
    }

    function approve() {
        $id = $this->uri->segment(4);
        $page = $this->uri->segment(5);
        $ubah = $this->story->edit_data(array('status' => 2), array('id' => $id));
        if ($ubah == true) {
            $this->session->set_flashdata('info_messages', 'Successfully Deleted');
        } else {
            $this->session->set_flashdata('info_messages', 'Fail Deleted');
    ...
    function reject() {
        $id = $this->uri->segment(4);
        $page = $this->uri->segment(5);
        $ubah = $this->story->edit_data(array('status' => 3), array('id' => $id));
        if ($ubah == true) {
            $this->session->set_flashdata('info_messages', 'Successfully Deleted');
        } else {
            $this->session->set_flashdata('info_messages', 'Fail Deleted');
